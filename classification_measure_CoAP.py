...
#####Inlcuido para realizar medicao de energia
from pyJoules.energy_meter import measure_energy
from pyJoules.handler.csv_handler import CSVHandler

csv_handler = CSVHandler('result_measure_coap.csv')

#####Abertura do arquivo para escrita, a finalidade será registrar o nome do algoritmo de ML em execução 
ArqClassificador = open('Algoritmo_ML_Execucao.csv','w')
    ...
    if os.path.getsize(path)//10 ** 9 > 0:
        ...
        for chunk in pd.read_csv(path, chunksize=10 ** 6):
            ...
            if one_hot_encoder == None:
                ...
                ct = ColumnTransformer([("is_attack", OneHotEncoder(),[0])], remainder="passthrough")                
                #x_temp = one_hot_encoder.fit_transform(x_temp).toarray()
                x_temp = ct.fit_transform(x_temp).toarray()
            else:
...
#            
        if mode == 0:
            dataset.drop(columns = columns_to_drop_packet, inplace = True, axis = 1)
        ...
        if mode == 0:
            ...
            if one_hot_encoder == None:
                #one_hot_encoder = OneHotEncoder(categorical_features=[0], n_values=30)
                #one_hot_encoder = OneHotEncoder(categorical_features=[0], categories=[range(28)] * 28)
                ct = ColumnTransformer([("is_attack", OneHotEncoder(),[0])], remainder="passthrough")                
...
def classify_sub(classifier, x_train, y_train, x_test, y_test, cm_file_name, summary_file_name, classifier_name, verbose = True):
    ...
    ArqClassificador.write(classifier_name+'\n')
...
@measure_energy(handler=csv_handler)
#def FuncaoLinear(random_state,x_train,y_train,x_test,y_test,confusion_matrix_folder,prefix):
def classify(random_state, x_train, y_train, x_test, y_test, folder_name, prefix = "", verbose = True):
    ...
    #FuncaoLinear(random_state,x_train,y_train,x_test,y_test,confusion_matrix_folder,prefix):
    # 2- KNN
...
if __name__ == "__main__":
    ...
    for slice_number in range(10):
        ...
        x, y = load_file(prefix + 'normal-2.csv', 
                         args.mode, 
                         0, 0, 
                         folder_name, 
                         slice_number,
        ...
        x_temp, y_temp = load_file(prefix + 'Scan_A-2.csv', 
                                   args.mode, 
                                   1, 1, 
                                   folder_name,
                                   slice_number,
        ...
        x_temp, y_temp = load_file(prefix + 'Scan_Su-2.csv', 
                                   args.mode, 
                                   1, 2, 
                                   folder_name,
                                   slice_number,
        ...
        x_temp, y_temp = load_file(prefix + 'Sparta-2.csv', 
                                   args.mode, 
                                   1, 3,
                                   folder_name,
                                   slice_number,
